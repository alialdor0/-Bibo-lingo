...
function Phase3({ word, allWords, onNext }) {
  const [chosen, setChosen] = useState(null);
  const opts = useRef(shuffle([word, ...getWrong(word, allWords, 3)])).current;
  const answer = (opt) => {
    setChosen(opt.id);
    if (opt.id === word.id) { speak(word.word); setTimeout(onNext, 900); }
  };
  return (
    <View style={ph.card}>
...
function Phase4({ word, allWords, onNext }) {
  const [chosen, setChosen] = useState(null);
  const opts = useRef(shuffle([word, ...getWrong(word, allWords, 3)])).current;
  const answer = (opt) => {
    setChosen(opt.id);
...
function Phase5({ word, allWords, onNext }) {
  const [chosen, setChosen] = useState(null);
  const opts = useRef(shuffle([word, ...getWrong(word, allWords, 3)])).current;
  const answer = (opt) => {
    setChosen(opt.id);
...
function Phase6({ word, lineText, onNext }) {
    ...
    <View style={ph.card}>
      ...
      <View style={ph.inputRow}>
        <TextInput style={[ph.input, done ? ph.inputOk : null]}
          value={typed} onChangeText={setTyped} onSubmitEditing={check}
...
export default function Story({ onLeave }) {
  const { lang, track, useInk, useEraser, usePage, addGems } = useApp();
  const T = (k) => t(k, lang);
    ...
    <SafeAreaView style={s.safe}>
      ...
      <ScrollView contentContainerStyle={s.content} showsVerticalScrollIndicator={false}>
        ...
        {showPron ? (
          ...
          phase === 3 ? <Phase3 word={word} allWords={allWords} onNext={nextPhase} /> :
          phase === 4 ? <Phase4 word={word} allWords={allWords} onNext={nextPhase} /> :
          phase === 5 ? <Phase5 word={word} allWords={allWords} onNext={nextPhase} /> :
          phase === 6 ? <Phase6 word={word} lineText={line.text} onNext={nextPhase} /> :
