...
export default function Dict({ onBack }) {
  const { lang, gems } = useApp();
  const T = (k) => t(k, lang);
  const [section, setSection] = useState(null);
  const [mode,    setMode]    = useState(null);
  const [exKey,   setExKey]   = useState(0);

  const reviewWords = DICT_WORDS.filter(w => w.status === 'review' || w.status === 'forgotten');
  const exWords     = section ? DICT_WORDS.filter(w => w.status === section) : reviewWords;
    ...
    <SafeAreaView style={s.safe}>
      ...
      <ScrollView contentContainerStyle={s.pageContent}>
        <View style={s.statsRow}>
          {SECTIONS.map(sec => {
            const count = DICT_WORDS.filter(w => w.status === sec.key).length;
            return (
        ...
        {SECTIONS.map(sec => (
          <SectionBlock key={sec.key} sec={sec} words={DICT_WORDS.filter(w => w.status === sec.key)} lang={lang} />
        ))}
