...
function CoopGame({ trackId, onEnd, addGems }) {
  ...
  const checkWord = () => {
    if (typed.trim().toLowerCase() !== currentWord.toLowerCase()) { setTyped(''); addSound('danger moment'); return; }
    setTyped(''); addSound('correct answer');
    const nextWord = wordIdx + 1;
    if (nextWord >= currentLine.length) {
      setShowPartner(true); addSound('mission start');
      setTimeout(() => {
        setShowPartner(false);
        setDone(prev => [...prev, lineIdx]);
        const nextLine = lineIdx + 1;
        nextLine >= lines.length ? setGameOver(true) : (setLineIdx(nextLine), setWordIdx(0));
      }, 2200);
