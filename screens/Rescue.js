...
import { t, RESCUE_WORDS } from '../data';
...
function WordCard({ word }) {
  const color = word.daysLeft === 0 ? '#c0392b' : word.daysLeft <= 2 ? '#FFB300' : '#2E8B57';
  const label = word.daysLeft === 0 ? 'Expires today!' : 'In ' + word.daysLeft + ' days';
  return (
...
function RescueGame({ words, onDone, addGems }) {
  ...
  const answer = (opt) => {
    ...
    if (correct) { setScore(s => s + 1); addGems(3); Animated.sequence([Animated.timing(celebAnim, { toValue: 1.2, duration: 150, useNativeDriver: true }), Animated.timing(celebAnim, { toValue: 1, duration: 150, useNativeDriver: true })]).start(); }
    else shake();
    setTimeout(() => { setChosen(null); idx + 1 >= q.length ? setDone(true) : setIdx(i => i + 1); }, 800);
  };
...
export default function Rescue({ onBack }) {
  const { lang, gems, addGems } = useApp();
  const T = (k) => t(k, lang);
  const [screen, setScreen] = useState('home');
  const [filter, setFilter] = useState('all');

  const urgent = RESCUE_WORDS.filter(w => w.daysLeft === 0);
  const soon   = RESCUE_WORDS.filter(w => w.daysLeft > 0 && w.daysLeft <= 2);
  const later  = RESCUE_WORDS.filter(w => w.daysLeft > 2);
  const gameWords = filter === 'urgent' ? urgent : filter === 'soon' ? soon : RESCUE_WORDS;
  ...
  if (screen === 'game') return (
    <SafeAreaView style={s.safe}>
      ...
      <ScrollView contentContainerStyle={s.pageContent}>
        <RescueGame words={gameWords} onDone={() => setScreen('home')} addGems={addGems} />
      </ScrollView>
    </SafeAreaView>
  );
  ...
  return (
    <SafeAreaView style={s.safe}>
      ...
      <ScrollView contentContainerStyle={s.pageContent}>
        <BiboMsg text={'You have ' + urgent.length + ' words expiring today! Rescue them now!'} color="#c0392b" />
        ...
        <TouchableOpacity style={[s.startBtn, { opacity: urgent.length > 0 ? 1 : 0.4 }]}
          ...
          <View>
            ...
            <Text style={s.startBtnSub}>{urgent.length} words expire today</Text>
          </View>
        ...
        <TouchableOpacity style={s.startBtnSecond} onPress={() => { setFilter('all'); setScreen('game'); }}>
          ...
          <View>
            ...
            <Text style={s.startBtnSub}>{RESCUE_WORDS.length} words</Text>
          </View>
          <Text style={s.startBtnArrow}>←</Text>
        </TouchableOpacity>

        {urgent.length > 0 ? <Text style={s.listLabel}>Today</Text> : null}
        {urgent.map(w => <WordCard key={String(w.id)} word={w} />)}
        {soon.length > 0 ? <Text style={s.listLabel}>Coming Soon</Text> : null}
        {soon.map(w => <WordCard key={String(w.id)} word={w} />)}
        {later.length > 0 ? <Text style={s.listLabel}>Later</Text> : null}
        {later.map(w => <WordCard key={String(w.id)} word={w} />)}
      </ScrollView>
    </SafeAreaView>
  );
}
